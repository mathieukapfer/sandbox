...
#if 0
static struct class hello_class_with_sysfs = {
	.name = CLASS_NAME,
	.owner = THIS_MODULE,
	.dev_groups = hello_groups,
};
#endif

// CDEV stuff
// ------------------------------------------------------------------------------
#define DEV_NAME		"hello_module"	// Device name in /dev
#define DEV_COUNT  1
...
static int __init helloBBB_init(void) {
  ...
  #if 0
  // Register the device driver with **sysfd**
  helloDevice = device_create(&hello_class_with_sysfs, NULL, MKDEV(0, 0) /* my_dev.devnum */, NULL, DEV_NAME);
  if (IS_ERR(helloDevice)) {               // Clean up if there is an error
    // class_destroy(&hello_class_with_sysfs);           // Repeated code but the alternative is goto statements
    unregister_chrdev_region(my_dev.devnum, DEV_COUNT);
    cdev_del(&my_dev.cdev);
    printk(KERN_ALERT "Failed to create the device\n");
    return PTR_ERR(helloDevice);
  ...
  //   - or cmd liek: 'echo toto > /dev/my_module'
