...
{ LOG_ENTER;
  ...
  u32 imr = at91_adc_readl(st, AT91_ADC_IMR);
	unsigned int reg;

  // FIX MATK: LINE COMMENTED to force the eoc to be triggered
	// status &= imr; 

#if 0
  // Here is the log that explains the FIX above:
  LOG_INFO("irq:%d", irq);
  LOG_INFO("status@%x:%x", st->registers->status_register, status);
  LOG_INFO("imr   @%x:%x", AT91_ADC_IMR, imr);
  LOG_INFO("status &= imr :%x",  status);
  LOG_INFO("st->num_channels:%x", st->num_channels);
  LOG_INFO("GENMASK(st->num_channels - 1, 0):%x", GENMASK(st->num_channels - 1, 0));

  // log output:
  // -----------
  /* drivers/iio/adc/at91_adc.c:362:at91_adc_rl_interrupt:called */
  /* drivers/iio/adc/at91_adc.c:369:at91_adc_rl_interrupt:status@1c:f000f */
  /* drivers/iio/adc/at91_adc.c:370:at91_adc_rl_interrupt:imr   @2c:10000 */
  /* drivers/iio/adc/at91_adc.c:374:at91_adc_rl_interrupt:status &= imr :10000 */
  /* drivers/iio/adc/at91_adc.c:375:at91_adc_rl_interrupt:st->num_channels:4 */
  /* drivers/iio/adc/at91_adc.c:376:at91_adc_rl_interrupt:GENMASK(st->num_channels - 1, 0):f */
  /* drivers/iio/adc/at91_adc.c:381:at91_adc_rl_interrupt:handle_adc_eoc_trigger NOT CALLED !!! */
#endif

	if (status & GENMASK(st->num_channels - 1, 0))
		handle_adc_eoc_trigger(irq, idev);
  else {
    LOG_INFO("handle_adc_eoc_trigger NOT CALLED !!!");
  }
	...
	if (status & AT91RL_ADC_IER_PEN) {
		...
		/* Conversion finished and we have a touchscreen */
...
{ LOG_ENTER;
	...
	if (st->caps->has_tsmr)
		...
		ret = request_irq(st->irq, at91_adc_rl_interrupt, IRQF_TRIGGER_RISING, // FIX MATK
				  pdev->dev.driver->name, idev);
	if (ret) {
