...
#define LOG_MODULO(mod, ...)  {                                  \
    ...
    if (count % mod == 0)  { LOG(KERN_INFO, __VA_ARGS__)  }      \
  }
...
static irqreturn_t at91_adc_rl_interrupt(int irq, void *private)
{
  ...
  unsigned int channel_mask = GENMASK(st->num_channels - 1, 0);

#if 0
	if ( (status & channel_mask ) == channel_mask )
#else
  if ( status & channel_mask )
#endif
    {
      /*
         All data are ready if (status & channel_mask ) is equal to channel_mask

            with  -  status:f000f   (four channels are ready i.e EOC bit is set),
                  -  channel_mask:f (four channels are enabled),
                  -  imr:8          => only EOC3 i.e. the last data trigger the interruption

         [EOCx means End Of Conversion of channel x]

         BUT is too dangerous to *not* acknowledge the interruption (by reading all the
         data that are ready) as the interruption handler will be called again immediatly
         and consume lot's of CPU while waiting all the data to be ready !

         So the condition to trigger eoc handler is
              "(status & channel_mask )"
         instead of
              "(status & channel_mask ) == channel_mask )"

         Side effect is that the data are just 1ms older than what we want. This also happend
         if some data has already been read during the previous period.
...
static int at91_adc_configure_trigger(struct iio_trigger *trig, bool state)
{
  ...
  u32 last_channel_bit_mask = 0;
	...
	if (state) {
      ...
      LOG_INFO("channel %x enabled (=> SR@%x:0x%x)",
               AT91_ADC_CH(chan->channel),
               AT91_ADC_CHSR,
               at91_adc_readl(st, AT91_ADC_CHSR)
               );
      /* patch MATK: begin */

      /* compute the last EOC bit */
      if ( last_channel_bit_mask < AT91_ADC_CH(chan->channel) ) {
        last_channel_bit_mask = AT91_ADC_CH(chan->channel);
      }

		}

    /* #if 0: disable IRQ in order to test transfer done by
     *        Peripheric DMA Channel of ADC (PDC)
     */
#if 0
    ...
    if (last_channel_bit_mask > 0) {
      ...
      at91_adc_writel(st, AT91_ADC_IER, last_channel_bit_mask);
      LOG_INFO("=> SR@%x:0x%x",
               AT91_ADC_CHSR,
               at91_adc_readl(st, AT91_ADC_CHSR)
               );

      /* Do not set the DRDY mask (first data ready trigger)
         as it will trigger the handler to early */
